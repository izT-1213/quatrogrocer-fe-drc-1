...
async function FetchUser(userId) {
  try {
    const response =
      await Axios.post //`https://api.quatrogrocer.one/quatro_user/search?user_id=${userId}`
...
async function CreateAddressFunc(
  ...
  try {
    ...
    const response = await Axios.post(
      // "https://api.quatrogrocer.one/quatro_address/create",
...
async function CheckoutProcess(user_id) {
  try {
    const response1 = await Axios.post(
      ...
      "https://localhost:5004/quatro_cart/push",
      { user_id: user_id }
    );
    ...
    if (response1.status === 200) {
      try {
        const response2 = await Axios.post(
          ...
          "https://localhost:5004/quatro_transaction/update",
          { user_id: user_id }
        );
        ...
        if (response2.status === 200) {
          try {
            const response = await Axios.post(
              ...
              "https://localhost:5004/quatro_transaction/checkout",
              { user_id: user_id }
            );
            console.log(response.data.result);
            return response.data.result;
...
async function DeleteCart(user_id) {
  try {
    await Axios.post(
      ...
      "https://localhost:5004/quatro_cart/delete",
      { user_id: user_id }
    );
  } catch (err) {}
}
...
async function PaidCheckout(user_id) {
  try {
    const response = await Axios.post(
      ...
      "https://localhost:5004/quatro_transaction/update_payment",
      { user_id: user_id }
    );
    return response;
  } catch (err) {
